...
# Import for API routes that need access to search
...
def api_blueprint_factory(lazy_init=False):
    """
    Create the API blueprint with optional lazy initialization
    
    Args:
        lazy_init: If True, defers heavy initializations until first API call
    ...
    if not lazy_init:
        ...
        # Set up lazy initialization on first request
        _initialized = False
...
def _initialize_core_modules():
    """Initialize PatternAnalyzer and search engine"""
    try:
...
# Create an api instance for backward compatibility with routes
api = Blueprint('api', __name__)
...
def create_app():
    ...
    app.register_blueprint(api_blueprint_factory(), url_prefix='/api')
