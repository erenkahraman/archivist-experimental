...

{
  ...
  "content_details": [
    { "name": "<string: Concise description of a key visual element/motif, e.g., 'Large hibiscus flower', 'Stylized leaf edge', 'Interlocking chain link'>", "confidence": <float between 0 and 1> }
    // 1 to 3 items MAXIMUM. Focus on the defining elements of the main_theme.
  ],
  "secondary_patterns": [
...
4.  **Key Elements:** List only 1-3 of the most prominent and defining visual motifs in `content_details`. Keep descriptions concise. *Do NOT include colors or detailed texture here.* Provide confidence for each element.
